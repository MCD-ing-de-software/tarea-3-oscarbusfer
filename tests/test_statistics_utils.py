...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        npt.assert_allclose(result, expected, rtol=1e-10, atol=1e-10)
        self.assertEqual(result.shape,(3,),"El resultado debe tener la forma esperada" )
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        arr = [2,4,6]
        result = utils.min_max_scale(arr)
        self.assertAlmostEqual(result.min(), 0.0, places=10, msg='Valor mínimo del resultado debería ser 0.0 ')
        self.assertAlmostEqual(result.max(), 1.0, places=10, msg='Valor mínimo del resultado debería ser 1.0 ')
